...
int main(int argc, char** argv)
{
	...
	char* domain = NULL;
	char webpage[2000] = "/";
	int index = string_find_char(host, '/');
	...
	{
		domain = (char*) malloc(sizeof((char) * index+1));
		string_copy(domain, host, index+1);
		string_copy(webpage, host + index, 2000);
	}
	...
	snprintf(header_buffer, 512, "GET %s HTTP/1.1\r\nHost: %s\r\n\r\n", page, domain);
	printf("Header %s\n", header_buffer);
